...
def runTimeStats(path: str) -> Tuple[Solution, timedelta]:
    start = datetime.now()
    data = InputData(path)
    solver = Solver(data, 42)
    tabu = TabuSearch(data, maxSeconds = 999, maxIterations = 30, neighborhoodEvaluationStrategy = "BestImprovement", neighborhoodTypes = ['Swap', 'Insertion', 'BlockK3', 'TwoEdgeExchange'])
...
if __name__ == "__main__":
    ...
    # solver = Solver(data, 42)
    # tabu = TabuSearch(data, maxSeconds = 600, maxIterations = 10, neighborhoodEvaluationStrategy = "BestImprovement", neighborhoodTypes = ['Swap', 'Insertion', 'BlockK3', 'TwoEdgeExchange'])
    # bestSol = solver.RunLocalSearch("greedy", tabu)
    ...
    for file in os.listdir(os.path.join(path, "Testinstanzen")):
        if file.endswith(".json"):
            ...
            sol, time = runTimeStats(os.path.join(path, "Testinstanzen", file))
            times.append((file, time.total_seconds(), sol.profit))
